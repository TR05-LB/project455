...
export function embedLSB(wav, payload, password, useECC = true) {
...
export function extractLSB(wav, password) {
    const { dataOff, dataSize } = parseWav(wav);
    const samples = dataSize / 2;
    const readBits = (nBits, startBit = 0) => {
    ...
    if (72 + bodyBits > samples)
        throw new Error("Truncated payload");
    const rawBits = new Array(bodyBits);
    for (let i = 0; i < bodyBits; i++)
        rawBits[i] = wav[dataOff + (72 + i) * 2] & 1;
