export function bytesToBits(b: ArrayLike<number>): number[] {
  const out: number[] = new Array(b.length * 8);
...
export function hammingEncode(bits: ArrayLike<number>): number[] {
  const out: number[] = [];
...
export function hammingDecode(bits: ArrayLike<number>): number[] {
  const data: number[] = [];
  for (let i = 0; i < bits.length; i += 7) {
    ...
    if (syndrome >= 1 && syndrome <= 7) arr[syndrome - 1] ^= 1;
